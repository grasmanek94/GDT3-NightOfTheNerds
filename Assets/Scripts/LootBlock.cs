...
public class LootBlock : MonoBehaviour {
    ...
    public enum LootBlockState { Empty, Mushroom, Goomba, Gun }
    ...
    void OnTriggerEnter2D(Collider2D other)
    {
        if (other.gameObject.tag == "Player")
        {
			...
			else if (state == LootBlockState.Gun)
			{
				this.GetComponents<AudioSource>()[0].Play();
				state = LootBlockState.Empty;
				Instantiate(Resources.Load("Prefabs/Gun"), new Vector2(this.transform.position.x, this.transform.position.y + 0.64f), Quaternion.identity);
			}
        }
    }
}
